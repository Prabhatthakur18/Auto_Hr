...
import { employeeApi, learningApi, type Course, type Employee } from '../services/api';
...
export const AssignCourseDialog: React.FC<AssignCourseDialogProps> = ({ course, onClose, onAssigned }) => {
  ...
  const handleAssign = async () => {
    ...
    setError('');
    try {
      const res = await learningApi.assignCourse(course.id, {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
      ...
      <div className="relative bg-white rounded-[32px] border border-orange-100/50 shadow-2xl w-full max-w-lg overflow-hidden animate-scale-in max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-orange-100/50 flex-shrink-0">
          <div>
            <h3 className="text-lg font-black text-slate-800">Assign Course</h3>
            <p className="text-xs text-slate-500 font-bold uppercase tracking-wider mt-0.5">{course.title}</p>
          </div>
          <button
            disabled={submitting}
            onClick={onClose}
